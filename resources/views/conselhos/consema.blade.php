<x-layout.orgaos bg="bg-home">
    <section class="max-w-[1200px] mx-auto relative z-50 my-10 lg:px-5 sm:px-3">
        <!-- SECTION PROFILE -->
        <x-conselhos.profile name="Conselho Municipal de Meio Ambiente" sigla="CONSEMA"
            img="https://anexos.saocristovao.se.gov.br/storage/files/18/portal/icon/conselhos/consema.png" />

        <!-- SECTION DESCRIPITON -->
        <div class="max-w-[1200px] flex gap-12 xl:mb-10 sm:flex-col sm:gap-4 sm:mb-0">
            <div class="flex flex-col w-full gap-4 mx-auto ">
                <div>
                    <x-title img="" h2="Sobre" p="Sobre o Conselho Municipal de Meio Ambiente"></x-title>
                    <p
                        class="text-justify sm:text-xs sm:first-letter:text-6xl dark:text-white first-letter:text-7xl first-letter:font-bold first-letter:mr-3 first-letter:text-yellow-400 first-letter:float-left text">
                        Fica instituído o Conselho Municipal do Meio Ambiente de que trata o art. 11 da Lei nº
                        408/2019, órgão colegiado autônomo de caráter consultivo, deliberativo e normativo do Sistema
                        Municipal do
                        Meio Ambiente - SIMMA, com as atribuições já definidas no art. 12 daquele Código Municipal do
                        Meio Ambiente.
                    </p>
                </div>

                <div>
                    <x-title img="" h2="Objetivos"
                        p="Objetivos do Conselho Municipal de Meio Ambiente"></x-title>
                    <ul class="flex flex-col gap-2 px-12 mt-2 list-disc dark:text-white ">
                        <li class="text-sm">Deliberar sobre a política ambiental do Município, aprovar o plano de ação
                            da SEMAP e acompanhar sua execução;
                        </li>
                        <li class="text-sm">Aprovar as normas, critérios, parâmetros, padrões e índices de qualidade
                            ambiental, bem como métodos para o uso dos recursos ambienteais do Município, observadas as
                            legislações estadual e federal;
                        </li>
                        <li class="text-sm">
                            Aprovar os métodos e padrões de monitoramento ambiental desenvolvidos pelo Poder Público e
                            pelo particular;
                        </li>
                        <li class="text-sm">
                            Conhecer dos processos de licenciamento ambiental do Município;
                        </li>
                        <li class="text-sm">
                            Analisar a proposta de projeto de lei re relevância ambiental de Iniciativa do Poder
                            Executivo, antes de ser submetida a deliberação da Câmara Municipal;
                        </li>
                        <li class="text-sm">
                            Acompanhar a análise e emitir parecer sobre os EIA/RIMA;
                        </li>
                        <li class="text-sm">
                            Apreciar, quando solicitado; termo de referência para elaboração do EIA/RIMA e decidir sobre
                            a conveniência de audiênca pública;
                        </li>
                        <li class="text-sm">
                            Estabelecer critérios básicos e fundamentados para a elaboração do zoneamento ambiental,
                            podendo referendar ou não a proposta encaminhada pelo órgão ambiental municipal competente;
                        </li>
                    </ul>
                </div>

                <div class="flex flex-col gap-3 mb-4">
                    <x-title img="" h2="Membros" p="Membros do Conselho Municipal de Meio Ambiente"></x-title>
                    <x-conselhos.membro position="mb-[-7rem]"
                        img="https://anexos.saocristovao.se.gov.br/storage/files/18/portal/profile/janine.jpg"
                        nome="Janine Menezes de Oliveira" cargo="Presidente"
                        bio="Janine Menezes de Oliveira é Arquiteta e Urbanista, formada em 2002 pela Universidade
                    Tiradentes, com especialização em Gestão Ambiental (2006). Exerceu a função de servidora pública
                    na Administração Estadual de Meio Ambiente (2004-2022) com experiência em licenciamento,
                    monitoramento e fiscalização ambiental. Ocupou o cargo de Gerente de Licenciamento Ambiental
                    entre abril de 2015 e janeiro de 2019." />
                </div>

                <div class="flex flex-col gap-3 mb-4">
                    <x-title img="" h2="Membros Titulares"
                        p="Membros Titulares do Conselho Municipal de Meio Ambiente"></x-title>
                    <x-conselhos.membro position="mb-[-3rem]"
                        img="https://coenterprises.com.au/wp-content/uploads/2018/02/male-placeholder-image.jpeg"
                        nome="Edilio José Soares Lima" cargo="Membro Titular"
                        bio="Possui graduação em Arquitetura e Urbanismo pela Universidade Tiradentes (2001), Especialização em Plantas Ornamentais e Paisagismo pela UFLA/MG (2006), e Mestrado em Educação - UNIT/SE (2017). Foi professor do Curso de Arquitetura e Urbanismo da Universidade Tiradentes (2010-2019), Arquiteto da Subsercretaria de Estado do Patrimonio Cultural - SUBPAC (2009-2014), e Superintendente do Instituto do Patrimonio Historico e Artistico Nacional - IPHAN em Sergipe (2017-2018), Atualmente é professor titular no Instituto Federal de Educação, Ciências e Tecnologia de Sergipe (2011), atuou como Arquiteto Analista (2012-2019), Diretor de Planejamento (2019 - 2021) e Diretor de Licenciamento e Controle (2021-2024) da Secretaria Municipal de Infraestrutura de São Cristóvão, é Vice-Presidente do CAU - Conselho de Arquitetura e Urbanismo de Sergipe. Tem experiência na área de Desenvolvimento Urbano, Paisagismo e Patrimônio Histórico." />
                    <x-conselhos.membro position="mb-[.3rem]"
                        img="https://anexos.saocristovao.se.gov.br/storage/files/18/portal/profile/robson.jpeg"
                        nome="José Robson Almeida Santos" cargo="Membro Titular"
                        bio="José Robson Almeida Santos, Membro Titular, Procurador Geral do Município" />
                    <x-conselhos.membro position="mb-[-7rem]"
                        img="https://anexos.saocristovao.se.gov.br/storage/files/18/portal/profile/daniella.jpg"
                        nome="Daniella Fraga de Andrade" cargo="Membro Titular"
                        bio="Coordenadora de Vigilância Ambiental em Saúde, Responsável Técnica Divisão de Zoonoses, Fiscal Sanitária, CRMV 0642. " />
                    <x-conselhos.membro position="mb-[-5rem]"
                        img="https://anexos.saocristovao.se.gov.br/storage/files/18/portal/profile/marcos.jpg"
                        nome="Marcos Deumares da Silva" cargo="Membro Titular"
                        bio="Especialista em Direito Eleitoral, Graduado em Direito pela Faculdade Estácio-SE, Graduado em Design Gráfico pela Universidade Tiradentes-SE, Servidor Público Federal do Tribunal Regional Eleitoral de Sergipe desde o ano de 1996. Atualmente ocupa a função de Assessor Jurídico de Juiz-Membro do TRE/SE. Assessorou os Juízes-Membro; Marcos Antônio Garapa de Carvalho (Juiz Federal), Raymundo Almeida Neto (Jurista), Clarisse de Aguiar Ribeiro Simas (Jurista) e Carlos Krauss de Menezes (Jurista)." />
                    <x-conselhos.membro position="mb-[-8rem]"
                        img="https://anexos.saocristovao.se.gov.br/storage/files/18/portal/profile/rafaella.png"
                        nome="Rafaella Nascimento Silva" cargo="Membro Titular"
                        bio="Formada em agroecologia pelo Instituto Federal de Sergipe Campus São Cristóvão, membra agricultura da Rede de agroecologia Plantar para a Vida." />
                    <x-conselhos.membro position="mb-[-11rem]"
                        img="https://anexos.saocristovao.se.gov.br/storage/files/18/portal/profile/karinne.jpg"
                        nome="Karinne Santiago Almeida" cargo="Membro Titular"
                        bio="Karinne Santiago Almeida, Membro Titular" />
                    <x-conselhos.membro position="mb-[-3rem]"
                        img="https://coenterprises.com.au/wp-content/uploads/2018/02/male-placeholder-image.jpeg"
                        nome="Elder Correia dos Santos" cargo="Membro Titular"
                        bio="Elder Correia dos Santos, Membro Titular" />
                </div>

                <div class="flex flex-col gap-3">
                    <x-title img="" h2="Membros Suplentes"
                        p="Membros Suplentes do Conselho Municipal de Meio Ambiente"></x-title>
                    <x-conselhos.membro position="mb-[-3rem]"
                        img="https://coenterprises.com.au/wp-content/uploads/2018/02/male-placeholder-image.jpeg"
                        nome="Luciana Souza Viana" cargo="Membro Suplente" bio="Luciana Souza Viana, Membro Suplente" />
                    <x-conselhos.membro position="mb-[-8rem]"
                        img="https://anexos.saocristovao.se.gov.br/storage/files/18/portal/profile/diego.png"
                        nome="Diego Araujo Oliveira Silva" cargo="Membro Suplente"
                        bio="Advogado, Graduado em Direito pela Universidade Tiradentes - UNIT (2012); Mestre em Desenvolvimento e Meio Ambiente pela Universidade Federal de Sergipe (2019); Especialista em Segurança Pública e Democracia pela Universidade Federal de Sergipe e Rede Nacional de Altos Estudos em Segurança Pública (2016); Atualmente é Subprocurador Geral na Procuradoria Geral do Município de São Cristóvão/SE (2024); Exerceu o cargo Assessor e Coordenador Jurídico do Órgão (2017-2024); e também de Subprocurador Geral do Município (2015 -2016); Foi Coordenador Jurídico de Meio Ambiente e Obras Públicas pelo mesmo Ente Público (2013 a 2015). Por fim, exerce a Presidência da Comissão de Direito Ambiental e Urbanístico da OAB/SE (2022-2024)." />
                    <x-conselhos.membro position="mb-[-7rem]"
                        img="https://anexos.saocristovao.se.gov.br/storage/files/18/portal/profile/nelson.png"
                        nome="Nelson dos Santos Filho" cargo="Membro Suplente"
                        bio="Servidor público, profissional da saúde, há um pouco mais de dezoito anos servindo ao município como agente de combate às endemias, atualmente na supervisão geral dos programas de combate a Dengue, Leishmaniose e Esquistossomose. Formação: Técnico em Contabilidade, Técnico em Vigilância em Saúde. Cursando a primeira graduação, aluno do curso Tecnologia em Agente Comunitário de saúde e Endemias, pela Faculdade UNINTER." />
                    <x-conselhos.membro position="mb-[-5rem]"
                        img="https://anexos.saocristovao.se.gov.br/storage/files/18/portal/profile/daniela.png"
                        nome="Daniela Souza Feitoza" cargo="Membro Suplente"
                        bio="Possui Graduação em Geografia Licenciatura pela Universidade Federal de Sergipe- UFS. Mestrado em Análise Regional/PPGEO/UFS, Doutorado em andamento na linha de pesquisa Análise Regional e Territórios Rurais PPGGEO/Universidade Federal de Santa Maria /UFSM- Rio Grande do Sul. Atualmente é professora no Departamento de Geografia/UFS, é pesquisadora do Grupo de Pesquisa Sobre Transformações no Mundo Rural-GEPRU/UFS. Atuou na Faculdade de Ensino Regional Alternativa (FERA). Atuou como Coordenadora Pedagógica de educação ambiental no Projeto Elos da Caatinga- IDESA-BRASIL/FUNBIO/GEF TERRESTRE ( 2021-2023), no curso Sementes da Caatinga no Monumento Grota do Angico- Poço Redondo/SE. Coordenou a Equipe de Campo do Projeto Água e Saneamento/FUNDECI/Banco do Nordeste, nos municípios de Gararu e Poço Redondo -SE. É consultora voluntária EaD na área de gestão de projetos, Cooperativismo , Associativismo, Microcrédito e Metodologias Ativas no Projeto Incentiva Rede Empreender ( Empreendedorismo Social e Feminino)- no município de Bagé- RS. Colaboradora no Grupo de estudos interdisciplinares e de pesquisas em relações internacionais -GEIPRI -UFSM. Desenvolve pesquisas nas linhas de Geografia Regional, e desenvolvimento (Territorial , Local ) , Microeconomia, nas áreas de Geografia Econômica, Geografia Rural, e política." />
                    <x-conselhos.membro position="mb-[-5rem]"
                        img="https://anexos.saocristovao.se.gov.br/storage/files/18/portal/profile/jielza.png"
                        nome="Jielza Correia Santos" cargo="Membro Suplente"
                        bio="Eu sou Jielza Correia Santos, uma mulher de 59 anos. Nasci em 25 de maio de 1965, na cidade de Aracaju, onde cresci e estudei até o ensino médio pedagógico no IERB ( escola normal), também fiz curso técnico em enfermagem na escola Dr. Augusto Leite( H.C.). Aos 20 anos, após casamento, migrei para São Cristóvão, onde criei meus três filhos e atualmente vejo meus netos crescerem. Vivendo nessa cidade, pude perceber, ao longo da minha vida, as questões relacionadas à busca por melhores condições de vida. Vítima de injustiça social, grave violação de direitos humanos básicos, decido então lutar, contando com apoio de movimentos sociais urbanos e do campo. Me torno assim, uma militante por direitos. Hoje, também sou educadora popular em saúde, atuo no controle social municipal, nos conselhos de saúde, SUAS, COMSEAN . Moro em um acampamento de reforma agrária, pratico a agricultura familiar sustentável e sou defensora da agroecologia como modo de vida." />
                    <x-conselhos.membro position="mb-[-11rem]"
                        img="https://anexos.saocristovao.se.gov.br/storage/files/18/portal/profile/cesar.png"
                        nome="Cesar Henrique Matos e Silva" cargo="Membro Suplente"
                        bio="Arquiteto e urbanista, graduado pela Universidade Federal da Bahia, atualmente atua como docente pelo Departamento de Arquitetura e Urbanismo da Universidade Federal de Sergipe (DAU/UFS), onde desenvolve atividades de ensino, pesquisa e extensão sobre espaço público, centralidade urbana, mobilidade urbana, entre outras áreas afins. Desde agosto de 2021 é diretor do Campus Laranjeiras. Conselheiro titular do Conselho de Arquitetura e Urbanismo de Sergipe (CAU-SE), gestão 2024-2026, onde coordena a Comissão de Ensino e Formação. Doutor pela Universidade Federal da Bahia e mestre pela Universität Kassel, Alemanha." />
                    <x-conselhos.membro position="center_top_-1rem"
                        img="https://anexos.saocristovao.se.gov.br/storage/files/18/portal/profile/bruno.jpg"
                        nome="Bruno Dórea Lemos" cargo="Membro Suplente"
                        bio="Formou-se em Biomedicina pela Universidade Tiradentes - UNIT no ano de 2004, especializando-se nessa mesma Universidade em Saúde Pública no ano de 2008. Atuou, também, na área de Vigilância Sanitária, passando a se dedicar no ramo do comércio de alimentação no ano de  2009. É proprietário do Seo Inácio Bistrô, o estabelecimento está localizado no bairro Inácio Barbosa. Atualmente, é Presidente da Associação de bares e restaurantes de Aracaju - ABRASEL, Presidente do Conselho Fiscal da Abrasel Nacional, Presidente do Pólo gastronômico de Sergipe e Conselheiro da Câmara de turismo da Fecomércio, estando sempre empenhado em realizar projetos e propostas que ofereçam uma melhor qualidade junto ao setor de turismo e de alimentação fora do lar, e, atuando sempre com ações beneficentes junto à instituições de caridade." />

                </div>

                <div class="flex flex-col gap-3">
                    <x-title img="" h2="Atas" p="Atas do Conselho Municipal de Meio Ambiente"></x-title>
                    <x-municipio.anexo
                        href="https://anexos.saocristovao.se.gov.br/storage/files/18/portal/anexos/ATA_DA_1__REUNIAO_ORDIARIA_DO_CONSEMA_2024.pdf"
                        title="Ata da 1ª Reunião Ordinária do CONSEMA"
                        desc="Aos vinte dias do mês de março de dois mil e vinte e quatro (20/03/2024) às nove horas, no Paço
                        Municipal, Praça São Francisco nº 11, Bairro Centro deste município, reuniram-se os membros do
                        Conselho Municipal do Meio Ambiente (CONSEMA)."
                        autor="Janine Menezes" />
                    <x-municipio.anexo
                        href="https://anexos.saocristovao.se.gov.br/storage/files/18/portal/anexos/ATA_DA_SELECAO_DOS_REPRESENTANTES_DE_ORGANIZACOES_DA_SOCIEDADE.pdf"
                        title="Ata da Seleção dos Representantes de Organizações da Sociedade"
                        desc="ATA DA SELEÇÃO DOS REPRESENTANTES DE ORGANIZAÇÕES DA SOCIEDADE
                        CIVIL ELEITOS PARA COMPOSIÇÃO DO CONSELHO DO MEIO AMBIENTE DE SÃO
                        CRISTÓVÃO/SE (CONSEMA);"
                        autor="Janine Menezes" />
                </div>
            </div>
        </div>
    </section>

</x-layout.orgaos>
