<div class="relative flex items-center justify-center group">
    @if ($href !== '')
        <a href="{{ $href }}"
            ...
            <i class="fa-solid fa-{{ $icon }} text-xs xl:text-[10px] lg:text-sm"></i>
        ...
        <span
            class="rounded-button-title absolute p-2 text-[10px] animate-arrow-bounce font-semibold opacity-100 text-yellow-800 bg-yellow-400 rounded-lg -bottom-12 whitespace-nowrap transition-all duration-500
                ...
                xl:text-[8px] lg:text-[11px] lg:-bottom-14">
            {{ $title }}
        </span>
    @else
        <button
            {{ $attributes->merge([
                'class' => 'relative z-30 overflow-hidden flex flex-col items-center gap-[3px] justify-center w-12 h-12 border-2 border-white rounded-full text-white
                                xl:w-10 xl:h-10 lg:w-14 lg:h-14 sm:w-10 sm:h-10',
            ]) }}>
            <i
                class="icon fa-solid fa-{{ $icon }}  @if ($icon === 'bars') text-yellow-800 xl:text-sm lg:text-2xl sm:text-sm  @else text-white text-xs group-hover:text-yellow-800 xl:text-[10px] lg:text-sm @endif"></i>
