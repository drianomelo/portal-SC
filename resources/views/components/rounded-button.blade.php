<div class="relative flex items-center justify-center group">
    ...
    @else
        <button
            {{ $attributes->merge([
                'class' => 'relative z-30 overflow-hidden flex flex-col items-center gap-[3px] justify-center w-12 h-12 border-2 border-white rounded-full text-white
                                xl:w-10 xl:h-10 lg:w-14 lg:h-14',
            ]) }}>
            <i
                class="fa-solid fa-{{ $icon }}  @if ($icon === 'bars') text-yellow-800 xl:text-sm lg:text-2xl lg:text-blue-950 @else text-white text-xs group-hover:text-yellow-800 xl:text-[10px] @endif"></i>

            <span
                class="absolute w-40 h-1 rotate-45 bg-yellow-100 blur-[3px] -left-10 -top-2 lg:bg-blue-600 @if ($icon === 'bars') animate-drop @else group-hover:animate-drop @endif"></span>
        </button>
        ...
        <span
            ...
            after:content-[''] after:absolute after:-z-10 after:w-4 after:h-4 after:-top-1 after:rotate-45 after:left-2/4 after:-translate-x-2/4 after:bg-yellow-400 xl:text-[8px] lg:text-[11px] lg:bg-blue-800 lg:after:bg-blue-800 lg:text-blue-950
